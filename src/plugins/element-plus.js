 ...
 * @LastEditors: hzf
 * @LastEditTime: 2022-05-19 17:31:49
 */
...
function findAnother(current, type, event) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    list.push({
      index: i,
      disabled: $typeOf(radio.className) === 'string' && radio.className.split(' ').includes('is-disabled'),
    });
...
function handleKeydown(e) {
  const classNames = $typeOf(e.target.className) === 'string' ? e.target.className.split(' ') : [];
  ...
  if ($typeOf(e.target.className) === 'string' && (e.target.className.includes('date_editor_wrap') ||
    e.target.className.includes('cascader_wrap'))) {
    if (e.keyCode == 13) {
